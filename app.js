/* =====================================================
   app.js v12b  （history → answerHistory に変更／安全化）
   ===================================================== */
...
let combo = 0;
let currentInput = "";
let answerHistory = [];   // ← ここを history から変更
...
els.submitBtn.onclick = () => {
  ...
  setTimeout(
    () => els.answerBox.classList.remove("answer-pop"),
    180
  );
  ...
  answerHistory.push({ a, b, ans, user, ok });   // ← history → answerHistory
  feedback("", ok);
  updateBuddy();
  updateComboUI();
  updateComboBgmSpeed();
...
function showResult(reason = "") {
  ...
  if (reason === "timeup") {
    msg =
      "じかんぎれ…でもここまでよくがんばったね！";
  } else if (reason === "gameover") {
    msg =
      "まちがいが3回になったよ。つぎはもっと気をつけてみよう！";
  } else if (score === 100) {
    msg =
      "ぜんもんせいかい！きょうりゅうも大よろこび！";
  } else if (score >= 80) {
    msg = "とてもいい点数！つぎは100点をめざそう！";
  } else if (score >= 60) {
    msg =
      "あとすこしで金メダル！もう一回やってみよう！";
  } else {
    msg =
...
// バックドロップの click で閉じる（null ガード付き）
