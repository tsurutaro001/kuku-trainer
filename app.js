...
let bgmSectionIndex = 0; // セクション番号

let nightMode = false; // タイトル長押しで ON／OFF
...
/* BGM 開始（AudioContext.resume を待ってから開始） */
function startBGM() {
  initAudio();
  if (!AC || !bgmGain) return;

  const doStart = () => {
    stopBGM(); // 前回のタイマー等を完全停止
    ...
    const barSec = first.melody.reduce(
